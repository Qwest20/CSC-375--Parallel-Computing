...
public class BlackPlayerTaskB extends Thread {
    ...
    // constructor
    public BlackPlayerTaskB(String n, WatchPlayLock w, Phaser phB) {
        name = n;
        wPL = w;
        ph = phB;
        ph.register();
    ...
    public void run() {
        ...
        try {
            ...
            while (!moved) {
                ...
                // i goes from 7 to 4
                outer:
                for (int i = BoardB.checkersGame.length - 1; i >= 4; i--) {
                    // j goes from 0 to 7
                    for (int j = 0; j < BoardB.checkersGame[0].length; j++) {
                        // we can manipulate the piece
                        if (BoardB.checkersGame[i][j] == 2) {
                            ...
                            if (movesThisPiece == 1) {
                                ...
                                if (i == 4 || BoardB.checkersGame[i - 1][j] != 0)
                                    up = false;
                                if (i == 7 || BoardB.checkersGame[i + 1][j] != 0)
                                    down = false;
